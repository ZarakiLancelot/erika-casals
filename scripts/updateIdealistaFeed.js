...
async function updateIdealistaProperties() {
  ...
  try {
    ...
    console.log(`✅ Descargado: ${format.toUpperCase()} en ${downloadedPath}\n`);
    ...
    const saleOutputPath = path.join(outputDir, 'idealista-properties-sale.json');
    parser.saveToJSON(saleProperties, saleOutputPath);
    
    // Guardar propiedades de alquiler
    const rentOutputPath = path.join(outputDir, 'idealista-properties-rent.json');
