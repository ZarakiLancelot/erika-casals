...
class IdealistaXMLParser {
  constructor() {
    ...
    this.enums = {
      ...
      TypologyType: ['home', 'chalet', 'countryhouse', 'garage', 'office', 'warehouse', 
                     'room', 'land', 'vacational', 'new_development', 'custom_ad', 
                     'storageroom', 'building'],
      AddressVisibility: ['show_address', 'only_street_name', 'hidden_address'],
      State: ['new', 'pending', 'active', 'inactive', 'error'],
      BuiltType: ['new_development', 'new_development_in_construction', 
                  'new_development_finished', 'second_hand_to_be_restored', 
  ...
  transformProperty(ad) {
    ...
    const modificationDate = ad.modification?.[0] ? new Date(parseInt(ad.modification[0])) : null;
    const creationDate = ad.creation?.[0] ? new Date(parseInt(ad.creation[0])) : null;
    ...
    return {
      ...
      rooms: parseInt(housing?.roomNumber?.[0] || housing?.bedroomNumber?.[0] || 0),
      ...
      features: {
        ...
        parkingAvailable: housing?.parkingSpace?.[0]?.hasParkingSpace?.[0] === 'true'
      },
  ...
  extractAddress(myAddress, listingAddress) {
    ...
    if (myAddress) {
      ...
      displayAddress = this.formatStreet(streetTypeId, streetName, streetNumber);
      ...
      coordinates = {
        ...
        longitude: coords?.longitude?.[0] ? parseFloat(coords.longitude[0]) : null
      };
      ...
      if (street && !listingAddress.street[0].reason) {
        ...
        displayAddress = this.formatStreet(streetTypeId, streetName, 
  ...
  formatStreet(typeId, name, number) {
    const streetTypes = {
      1: 'Acceso', 2: 'Alameda', 3: 'Alto', 4: 'Arco', 6: 'Autopista',
      7: 'Avenida', 12: 'Calle', 15: 'Callejón', 18: 'Camino', 24: 'Carretera',
      31: 'Paseo', 34: 'Puente', 36: 'Plaza', 40: 'Rotonda', 42: 'Travesía',
      45: 'Vía', 47: 'Desconocida', 51: 'Galería', 59: 'Bulevar', 60: 'Costanilla',
      61: 'Carril', 62: 'Cuesta', 63: 'Glorieta', 64: 'Pozo', 65: 'Prolongación',
      66: 'Puerta', 67: 'Pasaje', 68: 'Rambla', 69: 'Ronda', 70: 'Senda',
      71: 'Vereda', 72: 'Autovía', 73: 'Circunvalación', 74: 'Riera', 75: 'Impasse'
    };
  ...
  getMultimediaTag(tagCode) {
    const tags = [
      'unknown', 'kitchen', 'bathroom', 'living_room', 'bedroom', 'hall',
      'corridor', 'terrace', 'yard', 'facade', 'garden', 'views', 'plan',
      'details', 'flat_mates', 'empty_parking_space', 'used_parking_space',
      'input_output', 'store', 'rooms', 'reception', 'waiting_room', 'office',
      'archive', 'parking_space', 'attic', 'swimming_pool', 'dining_room',
      'entrance', 'land', 'urban_plot', 'surrounding_area', 'junk_room',
      'penthouse', 'balcony', 'building_work', 'press_photo', 'studio'
    ];
  ...
  saveToJSON(properties, outputPath) {
    ...
    console.log(`✅ ${properties.length} propiedades guardadas en ${outputPath}`);
  }
}
...
if (require.main === module) {
  ...
  const xmlFiles = fs.readdirSync(xmlDir).filter(f => f.endsWith('.xml') && !f.includes('_Agents'));
  ...
  const outputPath = path.join(__dirname, '../public/idealista-properties.json');
  
  console.log(`📄 Parseando: ${xmlFiles[0]}`);
  
  parser.parseXMLFile(xmlPath)
    .then(properties => {
