 ...
 * Tests Image01-Image02 según el sheet oficial de validaciones
 *
 * Uso: node test-properties-image01-image02.js
 *
...
const testResults = [];
const CACHE_FILE = 'test-properties-image01-image02-cache.json';
...
async function runTests() {
	console.log('📷 TESTS IMAGE01-IMAGE02 - IDEALISTA API VALIDATION');
	console.log('='.repeat(60));
	...
	try {
		...
		await executeOrUseCache(
			'Image01',
			'Crear imágenes para propiedad existente',
			202,
			() =>
				makeRequest(
					'POST',
			...
			'Enviar exactamente dos imágenes en el request'
		);

		// Image02 - Find all images
		await executeOrUseCache(
			'Image02',
			'Buscar todas las imágenes de la propiedad',
			200,
			() => makeRequest('GET', `/api/properties/${testPropertyId}/images`),
			'Buscar todas las imágenes para una propiedad que tiene imágenes'
		);
		...
		console.log('\n🎉 TESTS IMAGE01-IMAGE02 COMPLETADOS!');
		console.log('='.repeat(60));
		...
		const reportFile = `idealista-properties-image01-image02-${
		...
		let markdown = `# 📷 Tests Image01-Image02 - Idealista API

## 📊 Resumen
- **Fecha:** ${new Date().toLocaleDateString('es-ES')}
- **Tests:** Image01 a Image02 (Crear y buscar imágenes)
- **Total:** ${totalTests}
...
`;
		...
		const markdownFile = 'IDEALISTA_PROPERTIES_IMAGE01-IMAGE02_REPORT.md';
		fs.writeFileSync(markdownFile, markdown);
		console.log(`📋 Reporte Markdown: ${markdownFile}`);

		console.log('\n📧 PRÓXIMOS PASOS:');
		console.log('1. Revisar tests de validación fallidos');
		console.log('2. Continuar con tests Image03-Image06');
		console.log(
			'3. Verificar operaciones de actualización y eliminación de imágenes'
		);
	} catch (error) {
